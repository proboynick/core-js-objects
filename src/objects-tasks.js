...
function mergeObjects(objects) {
  return objects.reduce((resObj, currObj) => {
    const copy = { ...resObj };
    Object.entries(currObj).forEach(([key, value]) => {
      copy[key] = copy[key] ? copy[key] + value : value;
    });
    return copy;
  }, {});
}
...
function makeWord(lettersObject) {
  return Object.keys(lettersObject)
    .reduce((acc, currentKey) => {
      lettersObject[currentKey].forEach((position) => {
        acc[position] = currentKey;
      });
      return acc;
    }, [])
    .join('');
...
function sellTickets(queue) {
  const amount = queue.reduce((acc, value) => {
    let transAcc = acc;
    if (value > 25) {
      transAcc = acc - value;
    } else {
...
function Rectangle(width, height) {
  const resObj = { width, height };
  resObj.getArea = () => width * height;
  return resObj;
}
